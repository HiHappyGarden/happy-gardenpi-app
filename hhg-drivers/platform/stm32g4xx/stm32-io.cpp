...
inline void stm32_io::set_on_receive(const io_on_receive* obj, on_receive on_receive_callback) OS_NOEXCEPT
{
	this->obj = obj;
	this->on_receive_callback = on_receive_callback;
}

os::exit stm32_io::transmit(const uint8_t data[], uint16_t size) const OS_NOEXCEPT
{
	switch(source)
	{
	case LPUART:
